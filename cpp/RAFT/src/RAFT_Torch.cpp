...
RAFT_TorchScript::RAFT_TorchScript(){
    raft =std::make_unique<torch::jit::Module>(torch::jit::load("D:\\AA_CourseFile\\2021SummerIntern\\RAFT\\RAFT\\kitti.pt"));
    raft->to(at::kCUDA);
    //raft = std::make_unique<torch::jit::Module>(torch::jit::load("D:\\AA_CourseFile\\2021SummerIntern\\RAFT\\RAFT_Libtorch\\weights\\kitti.pt"));
...
Tensor RAFT_TorchScript::Forward_flow(Tensor& tensor0, Tensor& tensor1) {
    //auto result = raft->forward({tensor0,tensor1}).toTensorVector();
    auto result = raft->forward({ tensor0,tensor1 }).toTensor();
    return result;
}

Tensor RAFT_TorchScript::Forward_flow2(Tensor& tensor0, Tensor& tensor1) {
    //auto result = raft->forward({tensor0,tensor1}).toTensorVector();
    vector<torch::jit::IValue> input;
    input.push_back(tensor0);
    input.push_back(tensor1);
    auto result = raft->forward(input).toTensor();
    return result;
}
